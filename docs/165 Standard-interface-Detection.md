...
| Author   | [Christian Reitwießner](mailto:chris@ethereum.org), [Nick Johnson](mailto:nick@ethereum.org), [Fabian Vogelsteller](mailto:fabian@frozeman.de), [Jordi Baylina](mailto:jordi@baylina.cat), [Konrad Feldmeier](mailto:konrad.feldmeier@brainbot.com), [William Entriken](mailto:github.com@phor.net) |
| -------- | ------------------------------------------------------------ |
| Status   | Final                                                        |
| Type     | Standards Track                                              |
| Category | ERC                                                          |
| Created  | 2018-01-23                                                   |
| Requires | [214](https://eips.ethereum.org/EIPS/eip-214)                |
...
### How Interfaces are Identified 如何计算接口的 id
...
### How a Contract will Publish the Interfaces it Implements 如何发布接口的 id
...
> 求得异或结果后使用 keccak256 对结果进行加密，然后使用 bytes4 摘取前四字节
...
> 下面的代码介绍了如何查询一个合约是否实现了 165 提案，需要注意下面几点：
>
> - 查询分了三步：先查有没有 165 接口，再查 165 接口对不对，最后查提供的接口 id
> - 函数返回了多个值
> - noThrowCall 中使用了机器码
...
> 在下面的合约中，注册接口的查找是通过 map 结构来实现的
>
> 对函数名的访问是通过 selector 实现的
