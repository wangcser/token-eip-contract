...
| Author   | [William Entriken](mailto:github.com@phor.net), [Dieter Shirley](mailto:dete@axiomzen.co), [Jacob Evans](mailto:jacob@dekz.net), [Nastassia Sachs](mailto:nastassia.sachs@protonmail.com) |
| -------- | ------------------------------------------------------------ |
| Status   | Final                                                        |
...
> NFT 可以被个体用户拥有和使用，也可以交给第三方机构（经纪人，钱包，拍卖人）代为管理
>
> NFT 可以表示多元的资产
...
> 每一枚 NFT 都是可分辨的（互异的），因此合约必须跟踪每一枚 NFT
...
> 对于钱包应用来说，必须实现特定的接口才能实现代币合约定义的所有功能，这种对接受者的处理在代币合约中随处可见
...
> throw 可以理解为抛出异常的 event，下面是一些其他的扩展能力
...
> 这里对以太坊中的事件机制进行一个总结：
>
> 1. 由合约主动发出的事件采用 emit 机制，是合约执行结果的反馈
> 2. 由合约执行异常抛出的事件采用 throw 机制，是合约执行失败的反馈
>
> 事件机制方便了应用开发者个根据不同的事件设计应用
...
> 首先对这个接口进行分析，钱包等应用希望处理 721 代币的话需要实现这个接口，用于处理收到 NFT 这个事件。 721 合约会在向该地址转账后调用这个接口来判断转账是否成功，该接口被调用时，进行如下操作：
>
> 1. 能够接收该代币，则返回 magic value
> 2. 不能接收该代币，则抛出异常，代币合约回到初始状态
>
> 这里讨论的问题是这个回调接口是否需要 operator 字段，考虑到 operator 的行事能力，这里认为是必要的，但是我还需要再研究一下到底是为什么
...
> 这里是说，721 转账和 20 的区别在于 721 会接收转账的状态，而 20 只关心操作成功与否
...
4. ERC-223 Token Standard. https://eips.ethereum.org/EIPS/eip-223
5. ERC-677 `transferAndCall` Token Standard. https://eips.ethereum.org/EIPS/eip-677
6. ERC-827 Token Standard. https://eips.ethereum.org/EIPS/eip-827
